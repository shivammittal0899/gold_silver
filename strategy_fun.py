...
def generate_signals(df, p):
    ...
    df['green_cloudm'] = (df['senkou_a'] > df['senkou_b'])
    df['red_cloudm'] = (df['senkou_a'] <= df['senkou_b'])
    
    df['green_cloudf'] = ((df['senkou_af'] > df['senkou_bf'].shift(1)).rolling(3).sum() >= 3)
    df['red_cloudf'] = ((df['senkou_af'] <= df['senkou_bf'].shift(1)).rolling(3).sum() >= 3)
    
    df['green_cloudf'] = (df['senkou_af'] > df['senkou_bf'])
    df['red_cloudf'] = (df['senkou_af'] <= df['senkou_bf'])
    ...
    df['cloud_gapf'] = abs(df['senkou_af'] - df['senkou_bf'])
    df['cloud_gap_rangef'] = df['cloud_gapf'].rolling(5).max()
    ...
    df['kijun_move_down'] = (df['kijun'] < (df['kijun']).shift(1)).rolling(3).sum() >= 2
    ...
    df['entry_gap_short2'] = ((
                                ...
                                df['-DI_up'] &
                                df['-DI_move_up'] &
                                (df['senkou_b'] < (df['senkou_b'].shift(1) - df['ATR']*0.2)) &
                                (df['senkou_bf'] < (df['senkou_b'] - df['ATR'])) &
                                df['red_cloudf'] &
                                (df['price_below_tk']) 
                            ) & (~df['avoid_entry_short1']) )
    df['entry_gap_short1'] = ((
    ...
    df['entry_short_oi_vol'] = (
                                ...
                                (~df['avoid_entry1']))& (~df['avoid_entry_short2']) & (~df['avoid_entry_short3']) & (~df['avoid_entry_short4']) & (~df['avoid_entry_short7'])
    ...
    df['entry_short_pullback'] = (
        (df['Close'] < df['rolling_max_5'] - df['ATR']) &
        df['tenkan_below_kijun'] &
        # df['price_below_cloud'] &
        # df['price_below_tk'] &
        df['price_above_tk'].shift(2) &
        ((df['Close'] + df['ATR']*2) < cloud_bottom) &
        df['-DI_up'] &
        # (df['-DI'] > 25) &
        (df['DI_gap'] > df['DI_gap'].shift(1)*1.2) &
        ((df['Close'] < df['Open']).rolling(2).sum()==2)
    )
    ...
    df['exit_long_tkcross'] = (
                                ...
                                ) & (~df['avoid_entry_short1'])& (~df['avoid_entry_short2']) & (~df['avoid_entry_short6']) & (~df['avoid_entry_short3'])& (~df['avoid_entry1']) & (~df['avoid_entry_short7'])
    ...
    df['exit_long_cloud_exit'] = (
        df['red_cloud'] &
        ((df['Close'].shift(1) + 0)> df['senkou_a']) &
    ...
    ###################################
    df['avoid_exit_short1'] = (
        (((df['Close']+df['ATR']*0.5) < df['Open']).rolling(3).sum()>=2)
    )

    df['avoid_exit_short2'] = (
        df['green_cloud'] &
        df['green_cloudf'] &
        (df['Close'] < df['Open']) &
        (df['DI_gap'] < 8)
    ...
    df['exit_short_reverse_move'] = (
        ((df['Close'] > df['Open']).rolling(6).sum()>=4) &
        (df['High'] >= df['rolling_max_5']) &
        (df['Close'] >= df['rolling_max_5']) &
        df['price_above_tenkan'] &
        (df['rolling_min_5'] < df['tenkan']) &
        df['-DI_move_down'] &
        df['DI_gap_reduce'] 
    )
    ...
    df['exit_short_avoid'] = (
                              ...
                              ((df['rolling_max_10'] - df['rolling_min_10']) > (df['ATR']*0.6)))
    ...
    df['avoid_entry_long10'] = (
        (df['red_cloudf'].rolling(5).sum()==5) &
        (df['cloud_gap_rangef'] < df['ATR']*2.5)
    )
    df['avoid_entry_long11'] = (
        (df['red_cloudf'].rolling(5).sum()==5) &
        (df['cloud_gap_rangef'] < df['ATR']*2) &
        df['price_above_cloud']
    )
    df['avoid_entry_long12'] = (
        (df['range1'] < df['ATR']*1.5) &
        (df['candel_size'].rolling(5).mean() > df['candel_size'])
    )
    ...
    df['exit_short_tenkan_cross1'] = (
        
        (df['tenkan_below_kijun'] &
         df['price_below_cloud'] &
         (df['tenkan_kijun_gap'] > df['ATR']*2) &
         ((df['Close'] - 20) > df['tenkan']) &
         df['DI_gap_reduce'] &
         (df['-DI'] < 38) &
         (df['+DI'] > 10) &
         (df['RSI'] > 35) &
    ...
    df['exit_short_tenkan_cross2'] = (
        ...
        (df['tenkan_below_kijun'] &
         ...
         df['price_move_up'] &
         ((df['Close'] > df['Open']).rolling(3).sum()>=2) &
         (df['Close'] > df['rolling_min_5'] + df['ATR']*0.8) &
         df['price_below_tk'].shift(1))
        
    )
    df['exit_short_tenkan_cross3'] = (
        df['price_move_up'] &
        df['price_above_tenkan'] &
        (((df['Close'] - 0) > df['Open']).rolling(2).sum() >= 2) &
        df['-DI_move_down'] &
        ...
        #  df['price_move_up'] &
         df['tenkan_below_kijun'] &
         (df['tenkan_kijun_gap'] > df['ATR']*0.5) &
         (df['tenkan_kijun_gap'] < df['ATR']*1.5) &
         (df['Close'] > df['rolling_min_5'] + df['ATR']) &
         df['price_below_tk'].shift(2)) |
        
        (df['tenkan_below_kijun'] &
         (df['tenkan_kijun_gap'] > df['ATR']*1) &
         (df['Close'] > df['tenkan']) &
         (df['range1'] > df['ATR']*1.5) &
         df['DI_gap_reduce'] &
         (df['-DI'] < 35) &
    ...
    df['exit_short_price_cloud21'] = (
                                    ...
                                    (df['+DI'] > 10) & 
                                    (df['-DI'] < 39) &
                                    ((df['Close']+500) > df['tenkan']) &
                                    ((df['DI_gap'] < df['DI_gap'].shift(1)*1.15)) &
                                    (((df['High'] <= cloud_top) & df['tenkan_above_kijun'] & ((df['Close'] + 100) > df['Open'])) | 
                                     ((df['Close'] <= cloud_top) & (df['tenkan_below_kijun']) ))
                                    )
    df['exit_short_price_cloud22'] = (
                                    (((df['Close']-50) < df['Open']).rolling(3).sum() >= 3) & 
                                    (((df['Close']+150) > df['Open']).rolling(3).sum() >= 1) & 
                                    (df['RSI'] < df['rsi_ma']) &
                                    (df['RSI'] > df['rsi_ma']*0.5) &
                                    ((df['ADX'] > df['ADX'].shift(1)).rolling(3).sum() >= 2) &
                                    (df['RSI'] > 30) & 
                                    (df['+DI'] > 10) & 
                                    (df['-DI'] < 45) &
                                    ((df['Close']+500) > df['tenkan']) &
    ...
    df['exit_short_rsi_di1'] = (
        # df['price_below_tenkan'] &
        ((df['Close']-df['ATR']*0.1) < df['tenkan']) &
        (df['RSI'] >= 20) &
        (df['-DI'] > 42) &
        (df['-DI'] < 55) &
        (df['+DI'] < 15) &
    ...
    df['exit_short_rsi_di2'] = (
        # df['price_below_tenkan'] &
        ((df['Close']-df['ATR']*0.8) < df['tenkan']) &
        (df['RSI'] >= 20) &
        (df['-DI'] > 42) &
        (df['-DI'] < 55) &
        (df['+DI'] < 15) &
    ...
    df['exit_short_avoid1'] = (
                              (df['Close'] > df[['senkou_a','senkou_b']].min(axis=1)) &
                              (df['+DI'] > 17) &
                              (df['RSI'] > 38) &
                              (df['ADX'] > 15) &
                              (df['range1'] < df['ATR']*1.5) &
                              (df['range1'] > df['ATR']*0.7)
                            )
    df['exit_short_cloud_enter'] = (
        (((df['Close'].shift(1)-10) < df[['senkou_a','senkou_b']].min(axis=1)).rolling(2).sum() >=1) &
        ((df['Close']+20) > df[['senkou_a','senkou_b']].min(axis=1)) &
        # (df['Close'] > (df['rolling_min_5'] + df['ATR']*0.7)) &
        (df['green_cloud'] &
        ((df['Close'] + 10) > df['senkou_b']) &
        (df['price_move_up']) &
        (df['candel_size'] > 150)) 
    ...
    df['avoid_exit_long6'] = (
        ((df['Close'] > df['Open']).rolling(3).sum() == 3) &
        (df['DI_gap_reduce'] & (df['line_gap_reduce']) & df['+DI_move_up'])
    )
    df['avoid_exit_long7'] = (
        ((df['Close'] > df['Open']).rolling(4).sum() >= 3) &
        (df['DI_gap_reduce'] & (df['line_gap_reduce']) & df['+DI_move_up'])
    )
    df['avoid_exit_long8'] = (
        df['green_cloud'] &
        ...
        df['price_above_cloud'] &
        (df['Close'] > df['Open']) &
        ((df['line_gap'] < 50) | df['tenkan_above_kijun'])
    )
    ######
    ...
    df['entry_exit_indicator2'] = (
        ...
        (~df['avoid_exit_long5']) & (~df['avoid_exit_long6']) & (~df['avoid_exit_long8'])
    )
    df['entry_exit_indicator21'] = (
        (df['tenkan'] < df['kijun']) &
        ((df['-DI_up']) ) & (~df['avoid_exit_long']) & (~df['avoid_exit_long4'])  & 
        (~df['avoid_exit_long5']) & (~df['avoid_exit_long8'])
    )
    ...
    df['entry_exit_indicator23'] = (
        ((df['tenkan']+100) < df['kijun']) &
        (df['+DI'] < df['-DI']) & (~df['avoid_exit_long']) & 
        (~df['avoid_exit_long2']) & (~df['avoid_exit_long4'])  & 
        (~df['avoid_exit_long5']) & (~df['avoid_exit_long6'])
    )
    ...
    df['exit_long_price_gap'] = (
        (df['Close'] < (df['rolling_max_5'] - df['ATR']*1.5)) &
        ((df['Close'] < df['Open']).rolling(5).sum() >= 3) &
        (df['price_below_tenkan']) &
        (df['RSI'] < df['rsi_ma']) &
        df['-DI_up'] &
        ((df['DI_gap'] < 2).rolling(3).sum() >= 3)
    )
    ##
    df['exit_long_range_break'] = (
        df['price_below_tk'] &
        (df['Close'] == df['rolling_min_5']) &
        df['price_below_cloud'] &
        df['-DI_up'] &
        ...
        (df['+DI'] < 20) &
        (df['RSI'] < 50) &
        (df['RSI'] < df['rsi_ma']*0.98)
    )
    
    df['exit_long_range_break1'] = (
        df['price_below_tk'] &
        (df['Close'] == df['rolling_min_5']) &
        (df['Close'] <= df['rolling_max_5']-df['ATR']*1.5) &
        df['price_below_cloud'] &
    ...
    df['exit_short_sudden_up'] = (
        ...
        (df['DI_gap_reduce'].rolling(2).sum()==2) &
        df['-DI_move_down'] &
        ((df['Close']+100) >= df['rolling_max_5'].shift(1)) &
        ((df['High']+100) > df['kijun']) &
        (df['Volume'] > df['vol_ma'])
    )
    ...
    ##############
    ##############
    df['exit_long_final_end'] = (
        ((df['range2'] < df['ATR']*1.5) &
        df['price_above_cloud'] &
        (df['cloud_gap'] < df['ATR']*1.5) &
        (df['Close'] < (df['senkou_a'] + df['ATR']*2)) &
        (df['Close'] > (df['senkou_a'] + df['ATR']*1))) |
        
        ((df['range2'] < df['ATR']*2) &
        df['price_above_cloud'] &
        # (df['cloud_gap'] < df['ATR']*1.5) &
        (df['red_cloudf'] | df['red_cloud']) &
        (df['Close'] < (df['senkou_a'] + df['ATR']*2)) &
        (df['Close'] > (df['senkou_a'] + df['ATR']*0))) |
        (
            (df['Close'] > df['senkou_a']) &
            df['red_cloud'] &
            ((df['cloud_gapf'] < df['ATR']) | df['red_cloudf'])&
            (df['OI'] < df['oi_ma'])
        ) |
        (
            ((df['Close'] > df['Open']).rolling(4).sum() >=2) &
            ((df['Close'] > df['Open']).rolling(2).sum() >=1) &
            ((df['Close'] > (df['Open']-500)).rolling(3).sum() >=1) &
            (df['OI'] < df['oi_ma']) &
            df['price_above_tk'] &
            df['price_above_cloud'] &
            (df['Close'] < df['rolling_max_5']) &
    ...
    df['final_exit_short_end'] = (
        (
            ...
            (df['cloud_gap'] < df['ATR']*1.5) &
            df['price_above_tenkan'] &
            ((df['DI_gap'] < 6).rolling(3).sum()==3)
        ) |
        (
            ((df['Close'] > df['Open']).rolling(5).sum() >=2) &
            (df['cloud_gap'] < df['ATR']*2) &
            df['green_cloudm'] &
            df['red_cloudf'] &
            (df['+DI'] < 10) &
            (df['-DI_move_down'].rolling(3).sum()>=2) &
            (df['RSI'] > 30) 
        ) |
        (
            ((df['Close'] > df['Open']).rolling(5).sum() >=3) &
        ...
        (
            df['price_below_cloud'] &
            df['price_below_tk'] &
            (df['Close'] < df['tenkan'] - df['ATR']*1.5) &
            (df['cloud_gap'] < df['ATR']) &
            # (df['cloud_gapf'] < df['ATR']) &
            (df['DI_gap_reduce']) &
            (df['Close'] < (df['senkou_a'] - df['ATR']*2.5))
        ) |
        (
            df['green_cloud'] &
            df['green_cloudf'] &
        ...
        #     ((df['Close'] > df['Open']).rolling(5).sum() >=3) &
        #     (df['Close'] > (df['rolling_min_5'])) &
        #     (df['Close'] > (df['senkou_a'] - df['ATR']*5)) &
        #     # ((df['High'] > df['tenkan']).rolling(5).sum() >=2) &
        #     (df['range1'] > df['ATR']*1.5) &
        #     (df['range2'] > df['ATR']*1) &
        #     (df['cloud_gap'] < df['ATR']*1.5) &
        #     df['green_cloud'] &
    ...
    # df['df_exit_long_final'] = (df['entry_exit_indicator1'] | df['exit_long_below_tenkan_DI'] | df['exit_long_below_tk'] | df['entry_exit_indicator11'] | 
    #                       df['entry_exit_indicator2'] | df['entry_exit_indicator22'] | df['exit_long_gap_down'] |df['exit_long_move_down'] | 
    #                       df['trailing_entry_short0'] | df['entry_exit_indicator23'] | df['exit_long_range_break'] | df['exit_long_price_gap'] | 
    #                       df['entry_short_pullback'] | df['exit_long_cloud_exit'] | df['entry_exit_indicator3'] |  df['exit_long_price_drop'])
    
    # df['df_exit_long_final'] = (df['entry_exit_indicator1'] | df['exit_long_below_tenkan_DI'] | df['exit_long_below_tk'] | df['entry_exit_indicator11'] | 
    #                       df['entry_exit_indicator2'] | df['entry_exit_indicator22'] | df['exit_long_gap_down'] |df['exit_long_move_down'] | 
    #                       df['exit_long_range_break'] | df['exit_long_price_gap'] | ##df['entry_exit_indicator23'] | 
    #                       df['entry_short_pullback'] | df['exit_long_cloud_exit'] | df['entry_exit_indicator3'] |  df['exit_long_price_drop'])
    df['final_entry_long'] = (np.select(
        ...
        [   
            df['exit_short_above_cloud'] | df['exit_short_tenkan'] | df['exit_short_tenkan_cross1'],  ##  
            (df['exit_short_tenkan_cross2']),  ## df['exit_short_tenkan']
            df['exit_short_price_cloud21'] | df['exit_short_price_cloud'], 
            df['exit_short_indicators']  ## df['exit_short_price_cloud2'] | df['exit_short_above_cloud'] | 
        ],
        default=  df['exit_short_tenkan_cross3'] | df['exit_short_cloud_enter'] | df['exit_short_price_cloud22'] | df['exit_short_rsi_di1'] | df['exit_short_indicators']  ##  ##df['exit_short_tenkan'] | df['exit_short_price_cloud'] | df['exit_short_tenkan_cross']
    )| (df['entry_long_indicators'] | df['entry_long_indicators0'] | df['entry_long_indicators1'] | df['entry_long_price_move'] | df['entry_long_cloud_cross'] ))
    ...
    df_exit_long_final = (df['entry_exit_indicator1'] | df['exit_long_below_tenkan_DI'] | df['exit_long_below_tk'] | df['entry_exit_indicator11'] | 
                          df['entry_exit_indicator2'] | df['entry_exit_indicator22'] | df['exit_long_gap_down'] |df['exit_long_move_down'] | 
                          df['trailing_entry_short0'] | df['entry_exit_indicator23'] | df['exit_long_range_break'] | df['exit_long_price_gap'] | 
                          df['entry_short_pullback'] | df['exit_long_cloud_exit'] | df['entry_exit_indicator3'] |  df['exit_long_price_drop'])
    ...
    df['exit_long_final'] = np.select(
        ...
        [   
            df['entry_exit_indicator1'] | df['exit_long_below_tenkan_DI'] | df['exit_long_below_tk'] | df['entry_exit_indicator11'] | df['entry_exit_indicator2'] | df['entry_exit_indicator22'] | df['exit_long_gap_down'] |df['exit_long_move_down'] |df['trailing_entry_short0'] ,
            df['exit_long_below_tenkan_DI'] | df['exit_long_below_tk'] | df['entry_exit_indicator11'] | df['entry_exit_indicator2'] | df['entry_exit_indicator22'] | df['exit_long_gap_down'] |False,
            df['entry_exit_indicator1'] | df['exit_long_below_tenkan_DI'] | df['exit_long_below_tk'] | df['entry_exit_indicator11'] | df['entry_exit_indicator2'] | df['entry_exit_indicator22'] | df['exit_long_gap_down'] |df['exit_long_move_down'] |False,
            df['entry_exit_indicator2'] | False
        ],
        default=  df['entry_exit_indicator1'] | df['exit_long_below_tenkan_DI'] | df['exit_long_below_tk'] | df['entry_exit_indicator11'] | df['entry_exit_indicator23'] | df['entry_exit_indicator22'] | df['exit_long_gap_down'] |df['exit_long_move_down'] | (( df['trailing_entry_short0']  )& (~df['entry_long_avoid'])) ### | df['exit_long_price_cloud1']  df['exit_long_price_down'] |   ###| df['trailing_stop_long4'] 
    ) | (df['exit_long_range_break'] | df['exit_long_price_gap'] | df['entry_short_pullback'] | df['exit_long_cloud_exit'] | df['entry_exit_indicator3'] |  df['exit_long_price_drop'])
    

    # df['exit_long_final'] = (df['entry_short_pullback'] | df['exit_long_move_down'] | df['exit_long_below_tenkan_DI'] | df['exit_long_cloud_exit'] | df['exit_long_below_tk'] | df['entry_exit_indicator1'] | df['entry_exit_indicator11'] | df['entry_exit_indicator2'] | df['entry_exit_indicator22'] | df['entry_exit_indicator3'] | df['exit_long_gap_down'] | df['exit_long_price_drop'])
    ## df['exit_long_below_tenkan_DI'] | 
    ...
    final_unique_entry_short = (df['entry_gap_short'] | df['entry_gap_short1'] | df['entry_short_price_drop'] | 
                                ...
                                df['exit_long_price_kijun'] | df['entry_short_pullback'] | df['entry_short_price_bottom'] | df['exit_long_price_tk_cross3'] )
    ...
    df['final_entry_short'] = (np.select(
        ...
        [   
            ...
            df['exit_long_price_gap'] | df['exit_long_tkcross'] |  (( df['entry_gap_short'] | df['entry_short_price_bottom'] |df['trailing_entry_short1'] ) & (~df['entry_short_avoid'])) ,
            ((df['exit_long_price_kijun'] | df['exit_long_tkcross'] )& (~df['entry_short_avoid'])& (~df['entry_long_avoid'])),  
            ((df['exit_long_range_break1'] | df['exit_long_cloud_exit'] |df['entry_gap_short2'] |  df['exit_long_tkcross1'] |  df['entry_short_oi_vol']  ))  
        ],
        default=  ((df['entry_short_price_bottom'] | df['entry_short_price_drop']) ) 
    )| ( df['exit_long_price_tenkan'] |  df['entry_short_pullback'] |
        df['entry_short_price_top'] | df['entry_short_above_tk']  | df['exit_short_rsi_di2'] 
          )) 
    ...
    df['final_exit_short'] = np.select(
        ...
        [
            ...
            ((df['entry_long_below_cloud'] | df['exit_short_reverse_move']  )) ,  
    ...
    ) | (  df['exit_short_candle_green']  |  df['exit_short_price_pullback'] | df['exit_short_sudden_up'] |
...
def stopless_point(row, position, entry_price, prow):
    ...
    if position == 1 or position == 0:
        ...
        # if price_tenkan and price_cloud and row['price_move_up'] and row['+DI_up'] and (min5 > cloud_top) and (price_senkou_a_diff > atr*3) and ((price - min5) > atr):
        #     stoploss_value = min5
        if price_kijun and tenkan_kijun and row['+DI_move_down'] and (row['Close'] < row['Open']) and (tenkan_kijun_diff > atr*1.5):
        ...
        if (tenkan_kijun_diff > atr*3):
            ...
            if (kijun > senkou_a) and row['price_above_cloud'] and (row['cloud_gap'] > atr*3):
                ...
                if price_tenkan and (row['range2'] < atr*3) and (row['RSI'] < row['rsi_ma']) and (tenkan_kijun_diff > atr*4) and (min5 < tenkan) and (row['+DI'] < 40):
                    stoploss_value = min5 - atr*0.5
        ...
        if reverse_sl == 0:
            if price_cloud:
                if price_tenkan and (stoploss_value <= tenkan):
                    ...
                    if (row['line_gap_range'] < atr*2) and (abs(cloud_top - kijun) < atr*3):
        ...
        if not price_tenkan and tenkan_kijun and row['+DI_up'] and (tenkan_kijun_diff > atr*1) and (kijun > senkou_b) and (not senkou_a_b):
            ...
            reverse_sl = stoploss_value - atr*2
        if (row['line_gap_range'] < atr):
            reverse_sl = stoploss_value - atr
        if (not price_tenkan) and (not tenkan_kijun):
            reverse_sl = stoploss_value - 100
...
def stopless_point_short(row, position):
    ...
    if (not price_kijun) and not tenkan_kijun:
        if not price_tenkan:
            if (tenkan_kijun_diff > atr*2):
                ...
                if row['exit_long_final']:
                    ...
                    elif (price < row['senkou_af']) and (not price_tenkan) and ((open - price) > 500):
                        stoploss_val = open - 350
                    else:
                ...
                if row['exit_long_final']:
                    if row['+DI'] > 20:
                        ...
                        # stoploss_val = kijun + atr*0.2
                        # print(row['date'])
                        stoploss_val = min(tenkan + atr*1,kijun+atr*0.1)
                        if (row['line_gap_range'] < atr*1.5):
                            stoploss_val = min(tenkan + atr*1.2,kijun+atr*0.1)
                            # stoploss_val = 
                        # stoploss_val = kijun + atr*0.1
                    else:
        ...
        # elif (row['range1'] < atr) and (row['price_below_cloud']):
        #     stoploss_val = row['rolling_max_10'] + atr
    ...
    # if row['price_in_lines'] and (row['line_gap'] < 100):
    #     stoploss_val = max_line + atr*4
    # if 
        # stoploss_val = 0
...
def stoploss_entry_point(row, prow):
    ...
    if (not price_kijun) and not tenkan_kijun:
            ...
            if (tenkan_kijun_diff > atr*2):
                ...
                if row['exit_long_final']:
                    if row['+DI'] > 14:
                        ...
                        stoploss_val = tenkan + 50
                    else:
                        stoploss_val = kijun + 100
                if (row['cloud_gap'] < atr*0.2):
                    stoploss_val = kijun
                if (price < row['senkou_af']) and (not price_tenkan) and ((open - price) > 500):
                    stoploss_val = open - 250
            elif (tenkan_kijun_diff > atr*1):
                # print(row['date'], "---1")
                if row['+DI_up']:
                    stoploss_val = kijun + atr*0.2
                ...
                if row['+DI_up'] or (row['+DI_move_up'] and (row['DI_gap'] < 10)):
                    ...
                    stoploss_val = kijun + atr*0.4
                    if (row['-DI'] < 35):
                        stoploss_val = kijun + atr*0.0
        ...
        # print(row['date'])
        if price_tenkan_diff > atr:
            stoploss_val = price + atr*2
        elif (price_tenkan_diff <= atr) and (row['+DI'] > 25):
            stoploss_val = min(tenkan + atr*2, price + atr)
        elif (price_tenkan_diff <= atr) and (row['+DI'] <= 25):
            stoploss_val = tenkan + atr*2
            if price < min_cloud:
...
def wait_until_next_25min():
    """Wait until next 25-minute candle + 15 seconds mark."""
    now = datetime.now() + timedelta(hours=5, minutes=30)
    log(f"{now} -- wait for 25 minutes")
    # now = datetime.now()
    log(now)
    # Find the next 15-minute multiple
    min = 25
