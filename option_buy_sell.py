...
def process_index(kite_local, index_name,settings):
    ...
    try:
        ...
        with ThreadPoolExecutor(
            ...
            log2(f"latest date of ce - {ce_data['date']}")
            ...
            log2(f"latest date of pe - {pe_data['date']}")
...
def stoploss_value(option_data, settings, entry_price = 0):
    ...
    if sl_base == "tenkankijun":
        tenkan = option_data['kijun']
        kijun = option_data['kijun']
        tenkan_kijun_gap = abs(tenkan - kijun)
        if tenkan_kijun_gap > 7:
            sl_base_value = tenkan
        else:
            sl_base_value = kijun
    else:
